...
class Spline(nn.Module):
    ...
    def __init__(self, dim_spline: int, grid_size_spline: int = 5, order_spline: int = 3, grid_range: list = [-1, 1]):
        ...
        # OLD: self.base_weight = nn.Parameter(torch.Tensor(self.dim_spline, self.dim_spline))
        # FIXED: Initialize with a common method like Kaiming uniform or Xavier uniform
        self.base_weight = nn.Parameter(torch.empty(self.dim_spline, self.dim_spline))
        nn.init.kaiming_uniform_(self.base_weight, a=math.sqrt(5)) # Kaiming uniform for ReLU-like activations

        # Spline coefficients
        # OLD: self.spline_weight = nn.Parameter(torch.Tensor(self.dim_spline, self.dim_spline, self.grid_size_spline + self.order_spline))
        # FIXED: Initialize with a small uniform distribution
        self.spline_weight = nn.Parameter(torch.empty(self.dim_spline, self.dim_spline, self.grid_size_spline + self.order_spline))
        nn.init.uniform_(self.spline_weight, a=-0.01, b=0.01) # Small uniform values for spline coefficients
